...
<body style="width: 70%; margin:0 auto">
    ...
    $faqs=[
         ...
         [
            "domanda" => "ome faccio a rimuovere informazioni su di me dai risultati di ricerca di Google?",
    ...
    foreach($faqs as  $faq){
        ...
        foreach($faq as $key =>  $dati){
            ...
            if($key=="domanda"){
                ...
                $divisore = preg_split('#\R+#', $dati);
                foreach ($divisore as $dati){
                    echo '<p>'. $dati.'</p>';
                }
            }
        };
